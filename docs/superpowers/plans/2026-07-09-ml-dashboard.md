...
**Environment note (discovered during Task 2):** `vitest.config.ts` must be `vitest.config.mts` (not `.ts`) — the project has no `"type": "module"` in package.json, and `vite-tsconfig-paths` (needed for the `@/*` alias to resolve inside tests) is ESM-only; loading it from a config file resolved as CJS fails at config-load time. Renaming to `.mts` forces ESM config loading and fixes it. Separately, plain `import { DatabaseSync } from "node:sqlite"` fails to resolve under this Vitest/Vite version regardless of the config fix — any file that *instantiates* `DatabaseSync` (currently only `db/client.ts`) must load it via `const sqlite = require("node:sqlite") as typeof import("node:sqlite")` and call `new sqlite.DatabaseSync(...)`, while a plain `import type { DatabaseSync } from "node:sqlite"` (type-only, no instantiation) works fine anywhere else, including `db/tokens.ts` and later tasks (e.g. Task 8's `sync/sync-service.ts`) — do not combine a destructured `const { DatabaseSync } = require(...)` with `import type { DatabaseSync }` in the same file, it raises `TS2440` (duplicate identifier); use the namespace-style `const sqlite = require(...)` instead.
