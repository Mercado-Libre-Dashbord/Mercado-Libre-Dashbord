...
export async function GET(request: NextRequest) {
  ...
  const withMargin = await withScope({ accountId: account.id }, async (client) => {
    ...
    return rows.map((r) => ({
      ...r,
      marginPct: r.currentCost !== null && r.currentPrice > 0 ? (r.currentPrice - r.currentCost) / r.currentPrice : null,
    }));
  });
...
export async function PATCH(request: NextRequest) {
  ...
  await withScope({ accountId: account.id }, (client) =>
    client.query(`INSERT INTO product_costs (account_id, product_id, cost, valid_from) VALUES ($1, $2, $3, $4)`, [
      account.id,
      productId,
      cost,
